...
class BigQueryToCsv(Ingestion):
    ...
    def __init__(self, config, *args, **kwargs):
        ...
        self.credentials_path = Path(os.getcwd() + self.source_config["credentials"]["path"])
        self.credentials = GCPCredentials(
            path = self.credentials_path,
            kwargs= Json(
                name=self.source_config["credentials"]["name"], path=self.credentials_path
            ).get()
        )

        self.source = BigQuery(name=self.source_config["name"], credentials=self.credentials,path=self.source_config["path"])
